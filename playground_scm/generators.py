...
class SCMGenerator:
    ...
    def __init__(self, all_functions: Dict[str, Callable], seed: int, samples_shape: tuple, noise_std: float, noise_dist: str, nonlins: str, max_hidden_layers: int, nonlin_type: 'str'):
        """
    ...
    def create_scm_from_graph(self, graph: nx.DiGraph, possible_functions: List[str], exo_distribution: Callable,
        ...
        for n in graph.nodes:
            ...
            if graph.nodes[n]['type'] == 'endo':
                ...
                scm.add_endogenous_var(n, self.all_functions[current_function](parents=parents, samples_shape=self.samples_shape, noise_std=self.noise_std, noise_dist=self.noise_dist, nonlins=self.nonlins, max_hidden_layers=self.max_hidden_layers, nonlin_type=self.nonlin_type), {p: p for p in parents})
            else:
